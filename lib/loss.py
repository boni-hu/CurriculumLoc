...
def loss_function(
    ...
    for idx_in_batch in range(batch['image1'].size(0)):
        ...
        # Network output
        dense_features1 = output['dense_features1'][idx_in_batch] # shape [512,28,28] 1/8h,1/8w
        c, h1, w1 = dense_features1.size()
        scores1 = output['scores1'][idx_in_batch].view(-1) # [3844] h1*w1

        dense_features2 = output['dense_features2'][idx_in_batch]
        _, h2, w2 = dense_features2.size()
        scores2 = output['scores2'][idx_in_batch] # shape [62, 62] ## FIXME scores1和scores2的shape不一致

        all_descriptors1 = F.normalize(dense_features1.view(c, -1), dim=0)  # 512*3844
        descriptors1 = all_descriptors1

        all_descriptors2 = F.normalize(dense_features2.view(c, -1), dim=0)

        # Warp the positions from image 1 to image 2
        fmap_pos1 = grid_positions(h1, w1, device) # [2,3844]
        ...
        scores1 = scores1[ids]
        # Skip the pair if not enough GT correspondences are available 128
        if ids.size(0) < 128:
            continue
        ...
        # print("plot", plot)
        # print("batch idx:", batch['batch_idx'])
        # print("batch log_interval:", batch['log_interval'])
        # print("batch idx log_interval:", batch['batch_idx'] % batch['log_interval'])
        if plot and batch['batch_idx'] % batch['log_interval'] == 0:
            ...
            # model vit
            # im1 = imshow_image(
            #     preprocess_image(batch['image1'][idx_in_batch].cpu().numpy(), preprocessing=batch['preprocessing']),
            ...
            # model vit
            # im2 = imshow_image(
            #     preprocess_image(batch['image2'][idx_in_batch].cpu().numpy(), preprocessing=batch['preprocessing']),
            #     preprocessing=batch['preprocessing']
            # )
            im2 = imshow_image(

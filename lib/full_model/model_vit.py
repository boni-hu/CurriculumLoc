...
class DenseFeatureExtractionModule(nn.Module):
    ...
    def forward(self, batch):
        ...
        # transformer
        batch = self.feature_extractor(batch, return_tensors='pt')
...
class D2Net(nn.Module):
    def __init__(self, model_file=None, use_cuda=True):
        ...
        self.conv_out = nn.Conv2d(in_channels=768, out_channels=512, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, batch):
        ...
        # )
        dense_features1 = self.dense_feature_extraction(batch['image1'])[:,1:, :]
        dense_features2 = self.dense_feature_extraction(batch['image2'])[:,1:, :]
        dense_features1 = dense_features1.view(1, 14, 14, 768)
        dense_features2 = dense_features2.view(1, 14, 14, 768)
        # dense_features1 = dense_features1.permute(0,3,1,2)
        # dense_features2 = dense_features2.permute(0,3,1,2)
        print("dense_features1 max 1", dense_features1.max)
        print("dense_features1 min 1", dense_features1.min)

        # dense_features1 = self.conv_out(dense_features1)
        # print("dense_features1 shape conv", dense_features1.shape)
        # dense_features2 = self.conv_out(dense_features2)


        scores = self.detection(torch.cat([dense_features1, dense_features2], dim=0))
        print("scores shape:", scores.shape)
        # scores1 = self.detection(dense_features1)
        # scores2 = self.detection(dense_features2)


        # dense_features1 = dense_features[: b, :, :, :]
        # print("dense features:", dense_features1)
        print("feature max:", torch.max(dense_features1))
        print("feature min:", torch.min(dense_features1))

        # print("dense feaures 1 shape:", dense_features1.shape)  [1,512,28,28]

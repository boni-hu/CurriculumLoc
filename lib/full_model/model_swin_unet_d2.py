...
from ..backbone_model.swin_unet import SwinUnet

# FixMe 将特征提取的基础网络替换为swin-unet 其他计算scores计算匹配点对部分保持
class DenseFeatureExtractionModule(nn.Module):
    def __init__(self, finetune_feature_extraction=False, use_cuda=True):
        ...
        ## TODO Swin Unet
        trans_pretrained_path = '/home/a409/users/huboni/Projects/code/d2-net/models/swin_tiny_patch4_window7_224.pth'
        trans_pretrained_path = None
        self.model = SwinUnet(trans_pretrained_path).cuda()
        self.model.load_from(trans_pretrained_path)
...
class SoftDetectionModule(nn.Module):
    ...
    def forward(self, batch):
        b = batch.size(0)
        batch = F.relu(batch)
        max_per_sample = torch.max(batch.reshape(b, -1), dim=1)[0]
        # print("max pre sample:", max_per_sample)
        exp = torch.exp(batch / max_per_sample.reshape(b, 1, 1, 1))
...
class Swin_D2UNet(nn.Module):
    def __init__(self, use_cuda=True):
        super(Swin_D2UNet, self).__init__()
    ...
    def forward(self, batch):
        ...
        dense_features1 = self.dense_feature_extraction(batch['image1']) # [1,1000,224,224]
        # print("dense feature1:", dense_features1)
